...
export async function JsonDatabaseTests(_temp: PathLike) {
  ...
  try {
    ...
    assertFail(
      await database.addData("MANGA", testManga1),
      "Adding manga with dangling reference"
    );
    assertFail(
    ...
    assertFail(
      await database.updateData("MANGA", 2, testManga1),
      "Update the data so it similar for another entry"
    );
    assertFail(
    ...
    assertFail(
      await database.removeData("MANGA", 1),
